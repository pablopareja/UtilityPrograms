...
public class DetectRegionsByCoverageThresholdWig implements Executable {
    ...
    public void run(String[] args) {
        ...
        if (args.length != 5) {
            System.out.println("El programa espera 5 parametros: \n"
                    ...
                    + "5. Numero de contigs a analizar\n");
        } else {
            ...
            int numeroContigsAnalizar = Integer.parseInt(args[4]);
            int contadorContigsAnalizados = 0;
            ...
            try {
                ...
                while ( ((line = inBuff.readLine()) != null) && contadorContigsAnalizados < numeroContigsAnalizar) {
                    try{
                        int value = Integer.parseInt(line);
                        //Si he pasado esto es que estoy en una linea de numero de coverage
                        positions.add(value);
                        contigParsed = true;
                        
                    }catch(NumberFormatException ex){
                        //Estoy en una linea de variables
                        if (contigParsed) {
                ...
                if (contigParsed && contadorContigsAnalizados < numeroContigsAnalizar) {
                    ContigXML contig = new ContigXML();
                    contig.setId(lastHeader);
                    boolean hayGaps = analizaContig(contig, positions, umbral, longitudMinima);
                    outBuff.write(contig.toString() + "\n");
